...
<body>
    ...
    <banner>https://github.com/meshons/nn2018 ~ gabor.stork@sch.bme.hu</banner>
        ...
        <?php
            ...
            while($r = mysqli_fetch_assoc($sth)) {
                ...
                $sth2 = mysqli_query($con,"SELECT lastname,firstname,club,start FROM futok INNER JOIN start_3 ON futok.id = start_3.id WHERE category='".$r["category"]."' ORDER BY start");
    ...
    <?php
            ...
            while($r = mysqli_fetch_assoc($sth)) {
                ...
                $i = 1;
                $first = "";
                $sth2 = mysqli_query($con,"SELECT lastname,firstname,club,nap_2.time,nap_1.time+nap_2.time AS alltime FROM futok INNER JOIN nap_1 ON futok.id = nap_1.id INNER JOIN nap_2 ON futok.id=nap_2.id WHERE category='".$r["category"]."' AND nap_1.status=0 AND nap_2.status=0 ORDER BY alltime");
                 while($r2 = mysqli_fetch_assoc($sth2)) {
                 ...
                 $sth3 = mysqli_query($con,"SELECT lastname,firstname,club,nap_2.time FROM futok INNER JOIN nap_1 ON futok.id = nap_1.id INNER JOIN nap_2 ON futok.id=nap_2.id WHERE category='".$r["category"]."' AND nap_1.status<>0 AND nap_2.status=0 ORDER BY nap_2.time");
